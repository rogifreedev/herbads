...
import { runDueBatchChecks } from "@/lib/batches";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await runDueBatchChecks();
    return NextResponse.json(result);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Batch Daily Check konnte nicht ausgefuehrt werden.";
