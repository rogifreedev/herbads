...
import { generateWeeklyIterationsForAllClients } from "@/lib/creative-iterations";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await generateWeeklyIterationsForAllClients();
    return NextResponse.json(result);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Weekly Iterations konnten nicht generiert werden.";
