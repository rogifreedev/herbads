...
type CreativeAnalysisJobItemRow = {
  id: string;
  job_id: string;
  client_id: string;
  creative_id: string;
...
function staleRunningCutoffDate() {
  const minutesValue = Number(getOptionalEnv("CREATIVE_ANALYSIS_JOB_STALE_RUNNING_MINUTES", "15"));
  const minutes = Math.max(5, Math.floor(Number.isFinite(minutesValue) ? minutesValue : 15));
  return new Date(Date.now() - minutes * 60 * 1000).toISOString();
}

function maxAttempts() {
  const value = Number(getOptionalEnv("CREATIVE_ANALYSIS_JOB_MAX_ATTEMPTS", "3"));
  return Math.max(1, Math.floor(Number.isFinite(value) ? value : 3));
}

function batchSize() {
  const value = Number(getOptionalEnv("CREATIVE_ANALYSIS_JOB_BATCH_SIZE", "1"));
  return Math.max(1, Math.min(5, Math.floor(Number.isFinite(value) ? value : 1)));
}

function mapJob(job: CreativeAnalysisJobRow | null, items?: CreativeAnalysisJobItemRow[]) {
  if (!job) return null;
  ...
  return {
    ...
    activeItem: activeItem
      ? {
          creativeId: activeItem.creative_id,
...
async function resetStaleRunningItems(jobId: string) {
  ...
  const { error } = await supabase
    .from("creative_analysis_job_items")
...
async function syncJobProgress(job: CreativeAnalysisJobRow) {
  const supabase = createSupabaseServiceRoleClient();
  const { data: items, error } = await supabase
    .from("creative_analysis_job_items")
    .select("id,job_id,client_id,creative_id,item_index,status,attempts,analysis_id,error_message,run_after")
    .eq("job_id", job.id)
    .order("item_index", { ascending: true });

  if (error) throw new Error(error.message);

  const typedItems = (items ?? []) as CreativeAnalysisJobItemRow[];
  ...
  const nextRunAfter = pendingItems
    ...
    .sort()[0] ?? null;
  const shouldPause = unfinishedItems > 0 && Boolean(nextRunAfter) && new Date(nextRunAfter as string).getTime() > Date.now();
...
export async function getLatestCreativeAnalysisJobStatus(clientId: string) {
  const supabase = createSupabaseServiceRoleClient();
  ...
  return mapJob(job as CreativeAnalysisJobRow, (items ?? []) as CreativeAnalysisJobItemRow[]);
}

export async function startCreativeAnalysisJob(clientId: string, creativeIds: string[]) {
  const supabase = createSupabaseServiceRoleClient();
  const uniqueCreativeIds = Array.from(new Set(creativeIds.filter(Boolean)));
  if (uniqueCreativeIds.length === 0) throw new Error("Keine Creatives fuer die Analyse ausgewaehlt.");
  ...
  if (existing) return getLatestCreativeAnalysisJobStatus(clientId);
  ...
  if (validIds.length === 0) throw new Error("Keine gueltigen Creatives fuer diesen Partner gefunden.");

  const { data: job, error: jobError } = await supabase
    .from("creative_analysis_jobs")
    .insert({ client_id: clientId, status: "pending", total_items: validIds.length, payload: { source: "creative-library" } })
    .select("id")
    .single();

  if (jobError || !job) throw new Error(jobError?.message ?? "AI Analyse-Job konnte nicht erstellt werden.");

  const { error: itemError } = await supabase.from("creative_analysis_job_items").insert(
  ...
  return getLatestCreativeAnalysisJobStatus(clientId);
}
...
async function getNextActiveJob(clientId?: string) {
  ...
  if (error) throw new Error(error.message);
  return (data ?? null) as CreativeAnalysisJobRow | null;
}

export async function processNextCreativeAnalysisItems(clientId?: string) {
  const supabase = createSupabaseServiceRoleClient();
  const job = await getNextActiveJob(clientId);
  if (!job) return { status: clientId ? await getLatestCreativeAnalysisJobStatus(clientId) : null, processed: 0 };

  await resetStaleRunningItems(job.id);

  if (job.pause_until && new Date(job.pause_until).getTime() > Date.now()) {
    return { status: await getLatestCreativeAnalysisJobStatus(job.client_id), processed: 0 };
  }
  ...
  const pendingItems = (items ?? []) as CreativeAnalysisJobItemRow[];
  ...
  for (const item of pendingItems) {
    ...
    try {
      const analysis = await analyzeCreative(job.client_id, item.creative_id);
      await supabase
        .from("creative_analysis_job_items")
        .update({ status: "completed", analysis_id: analysis.id, finished_at: new Date().toISOString(), error_message: null })
        .eq("id", item.id);
      processed += 1;
    } catch (error) {
      const message = error instanceof Error ? error.message : "AI Analyse fehlgeschlagen.";
  ...
  await syncJobProgress(job);
  return { status: await getLatestCreativeAnalysisJobStatus(job.client_id), processed };
}
