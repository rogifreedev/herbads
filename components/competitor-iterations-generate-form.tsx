...
export function CompetitorIterationsGenerateForm({
  ...
  async function submit() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error ?? "Competitor Iterations konnten nicht generiert werden.");
      ...
      toast.success(created > 0 ? `${created} neue Competitor Iterations generiert.` : "Generation abgeschlossen, keine neuen Competitor Iterations erstellt.");
      startTransition(() => router.refresh());
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Competitor Iterations konnten nicht generiert werden.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
      ...
      <Button type="button" disabled={loading || pending} onClick={submit}>
        ...
        Competitor Iterations generieren
      </Button>
    </div>
  );
}
